...
char* indexes(const char* path)
{
    ...
    {
        // to html and php pointers, allocate enough memory to contain
        // the actual given path and the html/php string to be added
        // +1 for null terminator
        char* lengthhtml = malloc(strlen(path) + strlen("index.html") + 1);
        char* lengthphp = malloc(strlen(path) + strlen("index.php") + 1);
        
        //copy path into both pointers and strcat the appropriate endings
        strcpy(lengthhtml, path);
        strcpy(lengthphp, path);
        strcat(lengthhtml, "index.html");
        strcat(lengthphp, "index.php");
        
        //given the access to php (php exists), free html and return php
        if (access(lengthphp,F_OK) == 0)
        {
            free(lengthhtml);
            return lengthphp;
        }
        
        //  free memory and return html_length
        else if (access(lengthhtml,F_OK) == 0)
        {
            free(lengthphp);
            return lengthhtml;
        }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while(!feof(file))
    {
        ...
        if (bytes < 0)
        {
            //if it is not null, set it equal to null
            if (*content != NULL)
            {
                free (*content);
                *content = NULL;
            ...
            //set the pointer back to 0 and break for space
            *length = 0;
            break;
        }
        
        //reallocate memory to the pointer
        ...
        *length += bytes;
        ...
        //null terminate after all the contents
        *(buffer) = '\0';
    }
    return true;
}
...
const char* lookup(const char* path)
{
    ...
    if(path != NULL) 
    {
        //point to the first period, then point to the next index
        char* index = strrchr(path, '.');
        char* newpath = index + 1;
        
        //check conditions
        if(strcasecmp(newpath, "css") == 0)
        {
            return "text/css";
        }
        else if(strcasecmp(newpath, "html") == 0)
        {
            return "text/html";
        }
        else if(strcasecmp(newpath, "gif") == 0)
        {
            return "image.gif";
        }
        else if (strcasecmp(newpath, "ico") == 0)
        {
            return "image/x-icon";
        }
        else if (strcasecmp(newpath, "jpg") == 0)
        {
            return "image/jpeg";
        }
        else if(strcasecmp(newpath, "js") == 0)
        {
            return "text/javascript";
        }
        else if (strcasecmp(newpath, "php") == 0)
        {
            return "text/x-php";
        }
        else if (strcasecmp(newpath, "png") == 0)
        {
            return "image/png";
        }
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    //declar pointers for the first and second space
    char* index = strchr(line,' ');
    char* second = strrchr(line,' ');
    ...
    //copy 
    strncpy(method,line,index - line); 
    ...
    //request target will contain characters between 2 spaces
    char request_target[second - index + 1];
    request_target[second - index - 1] = '\0';
    strncpy(request_target, index +1 , second - index -1);
    ...
    //point to the last char, which is null, create HTTP-version
    char* null = strstr(line, "\r\n");
    char HTTP_version[null - second];
    HTTP_version[null - second - 1] = '\0';
    strncpy(HTTP_version, second + 1 , null - second - 1);
    
    
    //compare the copied string with HTTP/1.1
    ...
    //pointer to the question mark
    char* question = strchr(line,'?');
   
    //if the question mark does not exist
    if(question == NULL)
    {
        // without question mark, everything goes to abs_path, query does not exist
        strcpy(abs_path, request_target);
        query = "";
        return true;
    }
    //divide request_target to abs_path(before ?) and query(after ?)
    else 
    {
        //temp array to store question to second space
        char q[second - question + 1];
        q[second - question] = '\0';
        strncpy(q, question + 1, second - question);
        
        //another temp array to store first space to question
        char ab[question - index];
        ab[question - index - 1] = '\0';
        strncpy(ab, index + 1, question - index - 1);
        
        //copy to the necessary counterpart   
        strcpy(query, q);
        strcpy(abs_path, ab);
    }
    return true;
